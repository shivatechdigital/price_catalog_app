...
class _AdminReportsScreenState extends ConsumerState<AdminReportsScreen> {
          ...
          SliverToBoxAdapter(
            child: requirementsAsync.when(
              ...
              data: (requirements) {
                ...
                return _buildContent(requirements, products.length);
              },
            ),
          ),
        ],
  ...
  Widget _buildContent(List<RequirementModel> requirements, int productCount) {
    ...
    final totalValue = requirements
        ...
        .fold<double>(
          ...
          (sum, r) => sum +
              r.items.fold<double>(
                0,
                (s, i) => s + (i.traderOfferedPrice * i.quantity),
              ),
        );
    ...
    // Top traders
    final traderCounts = <String, int>{};
    for (final r in requirements) {
      traderCounts[r.traderName] = (traderCounts[r.traderName] ?? 0) + 1;
    }
    final topTraders = traderCounts.entries.toList()
      ..sort((a, b) => b.value.compareTo(a.value));
    ...
    return Column(
      ...
      children: [
        ...
        _SectionCard(
          title: 'Monthly Trend',
          subtitle: 'Last 6 months requirements',
        ...
        _SectionCard(
          ...
          subtitle: 'By requirements submitted',
          icon: Iconsax.people,
          child: topTraders.isEmpty
              ? _emptyState('No data yet')
              : Column(
                  children: topTraders
                      .take(5)
                      ...
                      .map(
                        (e) => _RankRow(
                          rank: e.key + 1,
                          label: e.value.key,
                          value: '${e.value.value} reqs',
                          color: AppColors.traderPrimary,
                          max: topTraders.first.value.toDouble(),
                          current: e.value.value.toDouble(),
                        ),
                      )
                      .toList(),
  ...
  Future<void> _exportPdf(List<RequirementModel> requirements) async {
    ...
    try {
      ...
      pdf.addPage(
        pw.MultiPage(
          ...
          build: (ctx) => [
            ...
            pw.SizedBox(height: 20),
            pw.Text('Recent Requirements (Last 20)',
                style: pw.TextStyle(
            ...
            pw.Table.fromTextArray(
              ...
              data: requirements.take(20).map((r) {
                return [
                  r.traderName,
                  r.customerName,
      ...
      final file = File(
          '${dir.path}/admin_report_${DateFormat('yyyyMMdd').format(now)}.pdf');
      ...
      if (mounted) {
        CustomSnackbar.showError(context, 'Failed to export PDF');
      }
    } finally {
  ...
  Future<void> _exportExcel(List<RequirementModel> requirements) async {
    ...
    try {
      ...
      _cell(summary, 0, 0, 'Metric', hStyle);
      _cell(summary, 0, 1, 'Value', hStyle);
      ...
      for (int i = 0; i < rows.length; i++) {
        _cell(summary, i + 1, 0, rows[i][0], null);
        _cell(summary, i + 1, 1, rows[i][1], null);
      }

      // ─── Requirements Sheet ───────────────────
      final reqSheet = excel['Requirements'];
      final headers = [
        'Trader',
        'Customer',
        'City',
      ...
      for (int c = 0; c < headers.length; c++) {
        _cell(reqSheet, 0, c, headers[c], hStyle);
      }
      final fmt = DateFormat('dd/MM/yyyy');
      for (int i = 0; i < requirements.length; i++) {
        final r = requirements[i];
        _cell(reqSheet, i + 1, 0, r.traderName, null);
        _cell(reqSheet, i + 1, 1, r.customerName, null);
        _cell(reqSheet, i + 1, 2, r.customerCity, null);
        _cell(reqSheet, i + 1, 3, r.status.name.toUpperCase(), null);
        _cell(reqSheet, i + 1, 4, r.paymentType.name, null);
        _cell(reqSheet, i + 1, 5, fmt.format(r.submittedAt), null);
      }

      final dir = await getTemporaryDirectory();
      final now = DateTime.now();
      final file = File(
          '${dir.path}/admin_report_${DateFormat('yyyyMMdd').format(now)}.xlsx');
      ...
      if (mounted) {
        CustomSnackbar.showError(context, 'Failed to export Excel');
      }
    } finally {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Icon(icon, size: 18.sp, color: AppColors.adminPrimary),
              Gap(8.w),
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
